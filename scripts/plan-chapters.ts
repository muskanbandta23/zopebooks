...
if (import.meta.main) {
  ...
  for (const ch of chapters) {
    ...
    console.log(`  ${ch.id} [${plan.density_level}]:`);
    console.log(`    Words: ${plan.word_target[0]}-${plan.word_target[1]}`);
    console.log(`    Sections: ${plan.sections.length}`);
    console.log(`    Visuals: ${visualCount} (${visualTypes})`);
    console.log(`    Content seeds: ${plan.content_seeds.key_claims.length} claims, ${plan.content_seeds.patterns.length} patterns`);
